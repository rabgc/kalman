    ...
    const Eigen::VectorXd &x) : S_(S), F_(F), Q_(Q), R_(R), P_(P), x_(x)
{
  ...
  assert(S_.rows() == S_.cols() && "State transition S must be square.");
  assert(S_.rows() == x_.size() && "State transition S and state vector x need same dim");
  assert(P_.rows() == P_.cols() && "Cov matrix P must be square");
  assert(P_.rows() == x_.size() && "Cov P matrix and state vector x need same dim");
  assert(Q_.rows() == Q_.cols() && "Process noise cov matrix Q must be square");
  assert(Q_.rows() == S_.rows() && "Process noise cov Q matrix and state transion S need same dim");
  assert(F_.cols() == x_.size() && "Features matrix F need same no. of cols as state vector x");
  assert(R_.rows() == R_.cols() && "Obs err cov matrix R must be square.");
  assert(F_.rows() == R_.rows() && "Features matrix F and obs err cov matrix R need same dim");
}
